...
│   │   ├── domain_service.go  # 域名业务逻辑
│   │   ├── mailbox_service.go # 邮箱业务逻辑
│   │   ├── email_service.go   # 邮件业务逻辑
│   │   ├── template_service.go # 模板业务逻辑
│   │   ├── signature_service.go # 签名业务逻辑
│   │   ├── rule_service.go    # 规则业务逻辑
│   │   ├── log_service.go     # 日志业务逻辑
│   │   ├── api_key_service.go # API密钥业务逻辑
│   │   └── draft_service.go   # 草稿业务逻辑
│   ├── model/                 # 数据模型层
│   │   ├── user.go            # 用户模型
│   │   ├── admin.go           # 管理员模型
│   │   ├── domain.go          # 域名模型
...
#### 3.3.2 分层职责规范
- **Handler层**: 仅处理HTTP请求/响应，调用Service层
- **Service层**: 核心业务逻辑，仅调用Model层方法
...
3. **依赖注入**：
   ...
   - Service层通过ServiceContext获取Model实例
...
### 主要特点
1. **分层清晰**：Handler -> Service -> Model 三层架构
