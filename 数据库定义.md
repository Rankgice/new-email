...
type EmailAttachment struct {
    Id        uint      `gorm:"primaryKey;autoIncrement" json:"id"`                         // 附件ID
    EmailId   uint      `gorm:"not null;index" json:"email_id"`                            // 邮件ID
...
type EmailTemplate struct {
    Id          uint           `gorm:"primaryKey;autoIncrement" json:"id"`                    // 模板ID
    UserId      uint           `gorm:"not null;index" json:"user_id"`                        // 用户ID
...
type EmailSignature struct {
    Id        uint           `gorm:"primaryKey;autoIncrement" json:"id"`                     // 签名ID
    UserId    uint           `gorm:"not null;index" json:"user_id"`                         // 用户ID
...
type UserVerificationRule struct {
    Id        uint      `gorm:"primaryKey;autoIncrement" json:"id"`                         // 关联ID
    UserId    uint      `gorm:"not null;index" json:"user_id"`                             // 用户ID
...
type OperationLog struct {
    Id         uint      `gorm:"primaryKey;autoIncrement" json:"id"`                         // 日志ID
    UserId     uint      `gorm:"index" json:"user_id"`                                      // 用户ID，0表示系统操作
...
type EmailLog struct {
    Id         uint       `gorm:"primaryKey;autoIncrement" json:"id"`                        // 日志ID
    MailboxId  uint       `gorm:"not null;index" json:"mailbox_id"`                         // 邮箱ID
    EmailId    uint       `gorm:"index" json:"email_id"`                                    // 邮件ID，0表示批量操作
...
type VerificationCode struct {
    Id          uint      `gorm:"primaryKey;autoIncrement" json:"id"`                        // 记录ID
    EmailId     uint      `gorm:"not null;index" json:"email_id"`                           // 邮件ID
    RuleId      uint      `gorm:"not null;index" json:"rule_id"`                            // 规则ID
...
type EmailDraft struct {
    Id          uint           `gorm:"primaryKey;autoIncrement" json:"id"`                    // 草稿ID
    UserId      uint           `gorm:"not null;index" json:"user_id"`                        // 用户ID
    MailboxId   uint           `gorm:"not null;index" json:"mailbox_id"`                     // 邮箱ID
    Subject     string         `gorm:"size:500" json:"subject"`                              // 邮件主题
    ToEmails    string         `gorm:"type:text" json:"to_emails"`                           // 收件人列表（JSON格式）
